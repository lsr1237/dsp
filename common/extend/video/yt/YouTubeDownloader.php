...
namespace common\extend\video\yt;
...
class YouTubeDownloader
{
    ...
    function __construct()
    {
        ...
        // $this->client->setProxy('127.0.0.1:10809');
    }
    ...
    public function getDownloadLinks($video_id, $selector = false)
    {
        ...
        $thumbnail = $json['microformat']['playerMicroformatRenderer']['thumbnail']['thumbnails'][0]['url'] ?? '';
        // do we want all links or just select few?
        if ($selector) {
            return [
                'thumbnail' => $thumbnail,
                'url' => $this->selectFirst($result, $selector)
            ];
        }
